...
export default function TodayPage() {
  ...
  async function fetchUserData() {
    try {
      ...
      const todayTodo = res2.data.data.filter((e) => e.categori === "nextweek");
  ...
  return (
    <section className="bg-white min-h-screen dark:bg-zinc-800 w-full border-r border-zinc-200 dark:border-zinc-800 py-20 sm:px-8 px-4">
      ...
      <AddTodoForm
        ...
        currentCategori="nextweek"
        ReRender={fetchUserData}
      />
      {loading === false && userTodo.length === 0 && (
        <div className="flex flex-col items-center justify-center h-[60vh] my-10">
          <LuCalendarDays className="text-[160px] text-zinc-300 dark:text-zinc-700" />
          <h3 className="text-4xl font-black">next 7 days is Empty</h3>
          <p className="text-zinc-400 dark:text-zinc-500">
            You dont have any tasks For next week
          </p>
        </div>
      )}
    </section>
